...
mkpath(log_dir)
mkpath(ckpt_dir)
log_file = joinpath(log_dir, "quench_from_CDW_to_LL_decrease_V_DV04_$(T_max)_centered_subsystem_l60.log")
checkpoint_file = joinpath(ckpt_dir, "quench_from_CDW_to_LL_decrease_V_DV04_$(T_max)_centered_subsystem_l60.h5")
...
pre_quench = SSHParams(v=1.0, w=1.0, Δ=0, V=2.2)
post_quench = SSHParams(v=1.0, w=1.0, Δ=0, V=1.8)
...
times, variances = simulate_quench_only_fluctuations(
    ...
    checkpoint_file=checkpoint_file, checkpoint_every=20, subsystem_a = 71, subsystem_b = 130
)
...
filename = "run_quench_from_CDW_to_LL_decrease_V_DV04_200sites_time$(T_max)_bonddim$(bond_dimension)_cutoff$(cutoff)_centered_subsystem_l60.csv"
