...
mkpath(log_dir)
mkpath(ckpt_dir)
log_file = joinpath(log_dir, "quench_from_LL_to_LL_bd1800_$(N_sites)decrease_V_DV02_$(T_max)_centered_subsystem_l80_new.log")
checkpoint_file = joinpath(ckpt_dir, "quench_from_LL_to_LL_bd1800_$(N_sites)_decrease_V_DV02_$(T_max)_centered_subsystem_l80_new.h5")
...
times, variances = simulate_quench_only_fluctuations_new(
    N_sites, T_max, dt, pre_quench, post_quench;
    bond_dim=bond_dimension, cutoff=cutoff, logfile=log_file,
    checkpoint_file=checkpoint_file, checkpoint_every=20, subsystem_a = 561, subsystem_b = 640, resume_from=checkpoint_file
)
...
filename = "run_LL_quench_from_LL_to_LL_decrease_V_DV02_sites$(N_sites)_time$(T_max)_bonddim$(bond_dimension)_cutoff$(cutoff)_centered_subsystem_l80_new.csv"
