...
mkpath(log_dir)
mkpath(ckpt_dir)
resumed_log_file = joinpath(log_dir, "quench_from_LL_to_LL_decrease_V_DV02_$(T_max)_centered_subsystem_RESUMED.log")
checkpoint_file = joinpath(ckpt_dir, "quench_from_LL_to_LL_decrease_V_DV02_$(T_max)_centered_subsystem.h5")
...
times, entropies, variances = simulate_quench(
    ...
    bond_dim=bond_dimension, cutoff=cutoff, logfile=resumed_log_file,
    checkpoint_file=checkpoint_file, checkpoint_every=20, resume_from=checkpoint_file, subsystem_a = N_sites ÷ 4 + 1, subsystem_b = 3 * N_sites ÷ 4
)
...
filename = "run_LL_quench_from_LL_to_LL_decrease_V_DV02_200sites_time$(T_max)_bonddim$(bond_dimension)_cutoff$(cutoff)_centered_subsystem.csv"
