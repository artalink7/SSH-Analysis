...
mkpath(log_dir)
mkpath(ckpt_dir)
log_file = joinpath(log_dir, "quench_from_CDW_to_LL_small_transition.log")
checkpoint_file = joinpath(ckpt_dir, "quench_from_CDW_to_LL_small_transition.h5")
...
pre_quench = SSHParams(v=1.0, w=1.0, Δ=0, V=2.5)
post_quench = SSHParams(v=1.0, w=1.0, Δ=0, V=1.5)
...
filename = "run_quench_from_CDW_to_LL_small_transition_200sites_10sec_bonddim$(bond_dimension)_cutoff$(cutoff).csv"
