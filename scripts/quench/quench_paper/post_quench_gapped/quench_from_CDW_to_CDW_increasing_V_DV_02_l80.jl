...
mkpath(log_dir)
mkpath(ckpt_dir)
log_file = joinpath(log_dir, "quench_from_CDW_to_CDW_increase_V_DV02_$(T_max)_centered_subsystem_l80.log")
checkpoint_file = joinpath(ckpt_dir, "quench_from_CDW_to_CDW_increase_V_DV02_$(T_max)_centered_subsystem_l80.h5")
...
pre_quench = SSHParams(v=1.0, w=1.0, Δ=0, V=4.0)
post_quench = SSHParams(v=1.0, w=1.0, Δ=0, V=4.2)
...
times, variances = simulate_quench_only_fluctuations(
    ...
    checkpoint_file=checkpoint_file, checkpoint_every=20, subsystem_a = 61, subsystem_b = 140
)
...
filename = "run_quench_from_CDW_to_CDW_increase_V_DV02_200sites_time$(T_max)_bonddim$(bond_dimension)_cutoff$(cutoff)_centered_subsystem_l80.csv"
