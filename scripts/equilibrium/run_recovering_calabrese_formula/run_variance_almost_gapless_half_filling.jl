...
for (i, n) in enumerate(N_cells)
    Variance[i] = SingleVariance([0, 1, 0.99999, 0], [0, 0.9999, 1, 0], "discrete", n, 2n, "1", 0)
    next!(p)
end

# Save Data
data_dir = joinpath(ProjectRoot(), "data", "equilibrium")
SaveWithTimeStamp(Variance, data_dir, "variance_vs_Ncells_almostgapless_half_filling")
...
savefig("plot/variance_vs_Ncells_almostgapless_half_filling.pdf")
