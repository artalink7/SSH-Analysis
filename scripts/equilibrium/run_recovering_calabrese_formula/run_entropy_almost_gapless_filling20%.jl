...
for (i, n) in enumerate(N_cells)
    Entropy[i] = EntanglementEntropy([0, 1, 0.99999, 0], [0, 0.9999, 1, 0], "discrete", n, 2n, "1", 0; filling_fraction=0.2)
    next!(p)
end

# Save Data
data_dir = joinpath(ProjectRoot(), "data", "equilibrium")
SaveWithTimeStamp(Entropy, data_dir, "entropy_vs_Ncells_almostgapless_filling20percent")
...
savefig("plot/entropy_vs_Ncells_almostgapless_filling20percent.pdf")
