...
Threads.@threads for i in eachindex(gap)
    ...
    S, V = EntropyandVariance_sub([0, 1 , 1, r/2], [0, 1 , 1, r/2], "discrete", N_cells, 2*N_cells, 0)
    Ratio_RM[i] = S / V
    #next!(p)
end 
...
SaveWithTimeStamp(Ratio_RM, data_dir, "ratio_entropy_variance_vs_gap_rm_1e-4_half_filling_5000cells")
